...
export default {
    ...
    ".equals(comparison)": async () => {
        ...
        return (succeedGood1 && succeedGood2 && succeedGood3 && succeedGood4 &&
    ...
    ".throws()": async () => {
        ...
        let threw = false;
        try {
            await expect(async () => true).throws();
        }
        catch (e) {
            threw = true;
        }
        if (!threw)
            throw "should have thrown because the function didn't throw";
    },
    ".not.throws()": async () => {
        const succeedGood1 = expect(() => true).not.throws();
        const succeedGood2 = expect(() => false).not.throws();
        const failGood1 = throwsError(() => expect(() => { throw new Error(); }).not.throws());
        const failGood2 = throwsError(() => expect(() => { throw "string"; }).not.throws());
        return (succeedGood1 && succeedGood2 && failGood1 && failGood2);
    },
};
